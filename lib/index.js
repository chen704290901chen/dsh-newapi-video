...
async function fetchOkBytes(url, headers) {
  const response = await fetch(url, headers ? { headers } : void 0);
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  return Buffer.from(await response.arrayBuffer());
}
// TOS/CDN 直链在本机常被代理掐 TLS。失败后改走中转站 GET /v1/videos/{id}/content，
// 与图片同一条中转链路，才能落到 newapi_output 并进资产库。不要让模型自己下载。
async function fetchVideoBytes(url, settings, taskId) {
  const errors = [];
  if (/^https?:\/\//i.test(url)) {
    try {
      return await fetchOkBytes(url);
    } catch (err) {
  ...
  if (id !== "") {
    try {
      ...
      return await fetchOkBytes(joinRelayPath(base, `/v1/videos/${encodeURIComponent(id)}/content`), {
        Authorization: `Bearer ${apiKey}`,
      });
    } catch (err) {
      errors.push(`relay: ${err?.message ?? err}`);
    }
  }
  throw new Error(`Video download failed (${errors.join("; ") || "no url"})`);
}
async function saveVideoToLibrary(url, settings, cwd, prompt, taskId) {
  const bytes = await fetchVideoBytes(url, settings, taskId);
  const dir = resolve(mediaRoot(settings, cwd), settings.outputDir || "newapi_output");
  await mkdir(dir, { recursive: true });
  const ts = new Date().toISOString().replace(/[-:T.]/g, "").slice(0, 14);
  const path = join(dir, `${slugify(prompt, 20)}-${ts}.mp4`);
  await writeFile(path, bytes);
...
async function generateVideo(settings, exec, args, cwd, onProgress) {
  ...
  for (const url of finished.urls) {
    files.push({ url });
    if (files.length === 1 && (/^https?:\/\//i.test(url) || cleanStr(finished.taskId) !== "")) {
      try {
        const path = await saveVideoToLibrary(url, settings, cwd, prompt, finished.taskId);
  ...
  if (downloadError !== "") result.downloadError = downloadError;
  return result;
}
  ...
  disposers.push(
    webServer.register({
      ...
      handler: async (request, response) => {
        ...
        const headers = {
          "content-type": mimeOf(extname(full)),
          "content-length": info.size,
          "cache-control": "no-store",
          "x-content-type-options": "nosniff",
        };
        if (download) headers["content-disposition"] = attachmentContentDisposition(basename(full));
