...
function readModelConfig() {
  try {
    ...
    if (ns && typeof ns === "object") return { baseURL: String(ns.baseURL ?? "").trim(), apiKeyRef: String(ns.apiKeyEnv ?? "").trim() };
  } catch {
...
async function resolveApiKey(settings) {
  ...
  if (ref !== "") {
    try {
      ...
      // fall through to the no-key error
    }
  }
...
async function resolveProviderApiKey(active, settings) {
  for (const env of active.env) {
    try {
      const credentials = pluginCtx?.get?.("credentials");
      const hit = await credentials?.resolve?.(env);
      const key = typeof hit?.key === "string" ? hit.key.trim() : (typeof hit?.value === "string" ? hit.value.trim() : "");
      if (key !== "") return key;
    } catch { /* try next */ }
  }
...
function sessionLedgerPath(project) {
  return join(resolve(project), ".dsh-newapi-session.json");
}
async function readSessionLedger(project) {
  const set = new Set();
  try {
    const raw = JSON.parse((await readFile(sessionLedgerPath(project), "utf8")).replace(/^\uFEFF/, ""));
    if (Array.isArray(raw)) for (const p of raw) if (typeof p === "string" && p !== "") set.add(p);
  } catch {
    // ledger missing or empty
  }
  return set;
}
async function addSessionFile(project, fullPath) {
  try {
    const rel = relative(resolve(project), resolve(fullPath)).replace(/\\/g, "/");
    if (rel === "" || rel.startsWith("..")) return;
    const set = await readSessionLedger(project);
    if (set.has(rel)) return;
    set.add(rel);
    await mkdir(dirname(sessionLedgerPath(project)), { recursive: true });
    await writeFile(sessionLedgerPath(project), JSON.stringify([...set], null, 0), "utf8");
  } catch {
...
export const Config = z.object({
  ...
  baseURL: z.string().default("").description("Relay base URL, including /v1. Empty defaults to the DSH model configuration."),
  apiKey: z.string().role("secret").default("").description("API key for the relay. Required (no environment-variable fallback)."),
  ...
  imageN: z.number().step(1).min(1).max(20).default(1).description("Default number of images to generate per call."),
...
async function openaiVideo(settings, exec, { prompt, options, media, cwd, onProgress }) {
  const base = effectiveBaseURL(settings).replace(/\/+$/, "");
  ...
  const createResponse = await fetch(`${base}${settings.submitPath}`, {
    method: "POST",
...
async function pollOpenAIVideo(settings, exec, { base, apiKey, taskId, onProgress }) {
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    const signal = signalOf(exec, settings.timeoutMs);
    const response = await fetch(`${base}${settings.statusPathTemplate.replace("{task_id}", encodeURIComponent(taskId))}`, {
...
async function modelverseVideo(settings, exec, { prompt, options, media, cwd, onProgress }) {
  ...
  const signal = signalOf(exec, settings.timeoutMs);
  const createResponse = await fetch(`${base}/tasks/submit`, {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
...
async function genericVideo(settings, exec, { prompt, options, media, cwd, onProgress }) {
  ...
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    const statusResponse = await fetch(`${base}${settings.statusPathTemplate.replace("{task_id}", encodeURIComponent(taskId))}`, {
      headers: { Authorization: `Bearer ${apiKey}` },
      signal: statusSignal,
    });
...
async function v1Video(settings, exec, { prompt, options, media, cwd, onProgress }) {
  ...
  const signal = signalOf(exec, settings.timeoutMs);
  const createResponse = await fetch(`${base}/v1/videos`, {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify(payload),
    signal,
...
async function pollV1Video(settings, exec, { base, apiKey, taskId, onProgress }) {
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolvePromise) => setTimeout(resolvePromise, settings.pollIntervalMs));
    const signal = signalOf(exec, settings.timeoutMs);
    const response = await fetch(`${base}/v1/videos/${encodeURIComponent(taskId)}`, {
      headers: { Authorization: `Bearer ${apiKey}` },
      signal,
    });
...
async function generateImages(settings, exec, args, cwd) {
  ...
  const n = clampInt(args.n ?? settings.imageN ?? 1, 1, 20, 1);
  ...
  disposers.push(
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Generate a video through the NewAPI relay station's video model. Use it when the user asks for a video, or references materials with @ (images/videos) plus a prompt. Pass @-referenced local file paths (or http(s)/data URLs) as first_frame (image-to-video first frame), last_frame, video (source video), or reference_images (appearance references). Resolves files against the session working directory.",
        parameters: {
          prompt: { type: "string", required: true, description: "Video prompt: concrete motion, subject, environment, style, lighting, lens." },
          first_frame: { type: "string", description: "Optional image path/URL for image-to-video (the first frame). Usually the @-referenced image." },
          last_frame: { type: "string", description: "Optional image path/URL for the last frame (first+last frame video)." },
          video: { type: "string", description: "Optional source video path/URL for video-to-video (mode-dependent)." },
          reference_images: { type: "array", items: { type: "string" }, description: "Optional reference image paths/URLs for character/appearance consistency." },
          aspect_ratio: { type: "string", description: "Aspect ratio such as 16:9, 9:16, 1:1." },
          duration: { type: "number", description: "Target duration in seconds (provider clamps unknown values)." },
          size: { type: "string", description: "Optional output size, e.g. 1280x720 (openai-videos)." },
          resolution: { type: "string", description: "Output tier, e.g. 720p/1080p (v1-videos, lowercase) or 2K/4k (modelverse-tasks)." },
          model: { type: "string", description: "Optional model id; overrides the configured model for this call." },
        },
        output: objectOutput(),
  ...
  disposers.push(
    ctx.tools.register(
      defineTool({
        ...
        description:
          "Generate or edit one or more images through the NewAPI relay station's image model. Without a reference image it POSTs an OpenAI-compatible /v1/images/generations; with @-referenced image(s) it POSTs a multipart /v1/images/edits to apply the prompt as edits. Saves the image(s) into the workspace image output directory, which also appear in the asset library.",
        parameters: {
          prompt: { type: "string", required: true, description: "Image prompt: subject, scene, style, lighting, composition, quality. For edits: the edit instruction (e.g. change color to blue)." },
          image: { type: "string", description: "Optional @-referenced source image path/URL to edit (image-to-image). Pass it to switch to the edits endpoint." },
          images: { type: "array", items: { type: "string" }, description: "Optional 1-3 @-referenced source images for multi-image editing." },
          n: { type: "number", description: "Number of images to generate (default from settings, max 20)." },
          size: { type: "string", description: "Image size, e.g. 1024x1024 (OpenAI) or 宽*高 e.g. 2048*2048 (qwen-image)." },
          quality: { type: "string", description: "Quality tier, e.g. auto/standard/hd/high/medium/low." },
          ...
          output_format: { type: "string", description: "Optional output format: png/jpeg/webp." },
          model: { type: "string", description: "Optional image model id; overrides the configured imageModel." },
        },
        output: objectOutput(),
        timeoutMs: generateTimeout,
        isConcurrencySafe: () => true,
        async execute(args, exec) {
          const cwd = sessionCwd(exec);
          return generateImages(settings(), exec, args, cwd);
        },
  ...
  disposers.push(
    ctx.tools.register(
      defineTool({
        ...
        parameters: {
          kind: { type: "string", description: "Optional filter: video | image | audio. Omit to list everything." },
        },
        output: objectOutput(),
        timeoutMs: 20000,
        isConcurrencySafe: () => true,
...
function registerGuidance(ctx, settings) {
  ...
  return ctx.systemPrompt.section({
    ...
    text: [
      ...
      "- 生成前若不确定(模型/画幅/时长/张数),先向用户确认,不擅自决定。默认:视频 5s/720p,图片 1 张。",
  ...
  disposers.push(
    webServer.register({
      ...
      handler: async (request, response) => {
        ...
        let models = [];
        try {
          const base = effectiveBaseURL(current).replace(/\/+$/, "");
