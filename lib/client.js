...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      { key: "baseURL", label: "Base URL", type: "text", hint: "中转站地址,带或不带 /v1 都可以;留空则用「模型配置」里的 baseURL" },
      { key: "apiKey", label: "API Key", type: "password", hint: "中转站 API Key。手动填过的会打码显示;留空则用「模型配置」里的 key" },
    ...
    function assetActionOf(asset) {
      const raw = typeof asset?.url === "string" ? asset.url.trim() : "";
      if (raw === "") return null;
      try {
        const url = new URL(raw, window.location.href);
        if (url.origin === window.location.origin && url.pathname === "/newapi/assets/file") {
          url.searchParams.set("download", "1");
          url.hash = "";
          return { kind: "download", href: url.toString() };
        }
        if (url.protocol === "http:" || url.protocol === "https:") {
          return { kind: "external", href: url.toString() };
        }
      } catch {
        // Invalid or unsupported URL: leave the asset preview-only.
      }
      return null;
    }

    function assetActionLabel(asset, action) {
      if (action?.kind === "external") return "打开临时链接";
      if (asset?.kind === "video") return "下载视频";
      if (asset?.kind === "audio") return "下载音频";
      return "下载原图";
    }

    function triggerAssetAction(asset) {
      const action = assetActionOf(asset);
    ...
    const GALLERY_CSS = [
      ...
      ".nva-action{opacity:0;transition:opacity .16s ease}",
      ".nva-cell:hover .nva-action,.nva-cell:focus-visible .nva-action{opacity:.94}",
      ".nva-cell .nva-action--busy,.nva-cell:hover .nva-action--busy{opacity:.4}",
    ...
    function AssetGrid(props) {
      ...
      const actionGroupStyle = { position: "absolute", top: 6, right: 6, display: "flex", alignItems: "center", gap: 5, zIndex: 3 };
      const quickBtnStyle = { width: 28, minWidth: 28, height: 28, padding: 0, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.24)", background: "rgba(0,0,0,0.66)", color: "#fff", cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center" };
            ...
            ? react.createElement(
                ...
                action
                  ? react.createElement(Button, {
                      type: "button",
                      className: "nva-action",
                      variant: "toolbar",
                      size: "sm",
                      style: quickBtnStyle,
                      icon: action.kind === "download" ? react.createElement(IconDownloadOutline16, { size: 16 }) : "↗",
                      title: assetActionLabel(asset, action),
                      "aria-label": assetActionLabel(asset, action),
                      onClick: (e) => {
                        e.stopPropagation();
                        triggerAssetAction(asset);
                      },
                    })
                  : null,
                onDelete
                  ? react.createElement(
                      ...
                      {
                        type: "button",
                        className: deleting ? "nva-action nva-action--busy" : "nva-action",
                        variant: "toolbar",
                        size: "sm",
                        style: quickBtnStyle,
                        title: "删除",
      ...
      if (view) {
        const v = view.url ?? "";
        const action = assetActionOf(view);
        const modalStyle = {
          position: "fixed",
          inset: 0,
          zIndex: 99999,
        ...
        const toolbarStyle = { width: "100%", minHeight: 48, padding: "8px 10px", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, background: "rgba(17,24,39,0.97)", color: "#fff" };
        const toolbarNameStyle = { flex: 1, minWidth: 0, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 12, fontWeight: 600 };
        const toolbarActionsStyle = { display: "flex", alignItems: "center", gap: 7, flexShrink: 0 };
        const toolbarBtnStyle = { color: "#fff", borderColor: "rgba(255,255,255,0.28)", background: "rgba(255,255,255,0.12)" };
        const closeBtnStyle = { ...toolbarBtnStyle, width: 32, minWidth: 32, height: 32, padding: 0, fontSize: 20, lineHeight: 1 };
        const mediaSize = { maxWidth: "min(72vw, 840px)", maxHeight: "min(calc(78vh - 48px), 572px)", display: "block", objectFit: "contain", background: "transparent" };
        let content;
        if (view.kind === "video") {
        ...
        lightbox = react.createElement(
          ...
          react.createElement(
            ...
            react.createElement(
              "div",
              { style: toolbarStyle },
              react.createElement("p", { style: toolbarNameStyle, title: view.name ?? "" }, view.name ?? "媒体预览"),
              react.createElement(
                "div",
                ...
                action
                  ? react.createElement(
                      Button,
                      {
                        ...
                        icon: action.kind === "download" ? react.createElement(IconDownloadOutline16, { size: 16 }) : "↗",
                        onClick: () => triggerAssetAction(view),
                      },
                      assetActionLabel(view, action),
                    )
                  : null,
                react.createElement(Button, { type: "button", variant: "toolbar", size: "sm", style: closeBtnStyle, "aria-label": "关闭预览", title: "关闭预览", onClick: () => setView(null) }, "×"),
              ),
            ),
            content,
          ),
    ...
    function NewapiVideoSection(scope) {
      return function Section() {
        ...
        const refreshKeyStatus = react.useCallback(async () => {
          try {
            const r = await fetch("/newapi/settings-status");
            setKeyStatus(r.ok ? await r.json() : null);
          } catch {
            setKeyStatus(null);
          }
        }, []);
        ...
        react.useEffect(() => {
          refreshKeyStatus();
        }, [refreshKeyStatus, snapshot.revision]);
        ...
        async function handleSave() {
          ...
          for (const field of FIELDS) {
            if (field.key === "apiKey") {
              if (!apiKeyTouched) continue;
              const next = normalize(field, draft[field.key]);
              if (next === void 0) continue;
              const current = normalize(field, value[field.key] ?? "");
              const pluginHadKey = keyStatus?.apiKeySet === true || current !== "";
              if (!Object.is(next, current) || (next === "" && pluginHadKey)) ops.push({ field, next });
              continue;
            }
            const next = normalize(field, draft[field.key]);
            if (next === void 0) continue;
        ...
        const rows = FIELDS.filter((field) => !field.onlyFor || draft.provider === field.onlyFor).map((field) => {
          ...
          if (field.key === "apiKey") {
            const snapshotKey = typeof value.apiKey === "string" ? value.apiKey.trim() : "";
            const pluginSet = keyStatus?.apiKeySet === true || snapshotKey !== "";
            const source = pluginSet ? "plugin" : (keyStatus == null ? "loading" : (keyStatus.apiKeySource ?? "none"));
            const maskedFromStatus = typeof keyStatus?.apiKeyMasked === "string" ? keyStatus.apiKeyMasked : "";
            const masked = maskedFromStatus || (snapshotKey.length === 0 ? "" : snapshotKey.length <= 8 ? "••••••••" : `${snapshotKey.slice(0, 3)}••••${snapshotKey.slice(-4)}`);
            const sourceLine =
              source === "plugin"
                ? `当前使用：本插件手动填写的 API Key（${masked || "••••••••"}）`
                : source === "model-config"
                  ? "当前使用：「模型配置」里的 API Key（本插件未手动填写）"
                  : source === "loading"
                    ? "正在读取 Key 来源…"
                    : "当前未配置 API Key（本插件和「模型配置」都没有）";
            const inputProps = {
              key: field.key,
              type: "password",
              disabled: !writable,
            ...
            if (apiKeyTouched) {
              inputProps.value = draftValue === undefined || draftValue === null ? "" : String(draftValue);
              inputProps.placeholder = "留空并保存 = 改用「模型配置」的 key";
            } else if (pluginSet) {
              // Put the mask in `value` so the password box looks filled, not blank.
              inputProps.value = masked || "••••••••";
              inputProps.placeholder = "";
            } else {
              inputProps.value = "";
              inputProps.placeholder = source === "model-config"
                ? "未手动填写，正在用「模型配置」的 key"
                : source === "loading"
                  ? "读取中…"
                  : "未设置";
            }
            const clearBtn =
              pluginSet && !apiKeyTouched && writable
                ? react.createElement(
                    "button",
                    {
                      type: "button",
            ...
            return react.createElement(
              ...
              clearBtn,
              react.createElement("p", { style: { ...hintStyle, color: source === "none" ? "var(--dsw-alias-state-warn-primary, #d97706)" : "var(--dsw-alias-label-secondary)" } }, sourceLine),
              field.hint ? react.createElement("p", { style: hintStyle }, field.hint) : null,
            );
          }
          const input = react.createElement("input", {
            ...
            disabled: !writable,
            placeholder: field.key === "baseURL"
              ? (keyStatus?.baseURLSource === "model-config" && !(String(draftValue || "").trim())
                ? (keyStatus.modelConfigBaseURL || "留空则用「模型配置」")
                : "例如 https://llm.example.com 或 …/v1")
              : undefined,
            style: inputStyle,
            onChange: (event) => setDraft((d) => ({ ...d, [field.key]: event.target.value })),
          });
          const baseSourceLine =
            field.key === "baseURL"
              ? (keyStatus?.baseURLSource === "plugin"
                ? "当前使用：本插件手动填写的 Base URL（带或不带 /v1 均可）"
                : keyStatus?.baseURLSource === "model-config"
                  ? `当前使用：「模型配置」的 Base URL（${keyStatus.modelConfigBaseURL || "已配置"}）`
                  : "当前未配置 Base URL")
              : null;
          return react.createElement(
            "div",
            ...
            react.createElement("label", { style: labelStyle }, field.label),
            input,
            baseSourceLine ? react.createElement("p", { style: hintStyle }, baseSourceLine) : null,
            field.hint ? react.createElement("p", { style: hintStyle }, field.hint) : null,
          );
        });
