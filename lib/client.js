...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      { key: "imageN", label: "默认生图张数", type: "number", hint: "1-20" },
    ...
    // 统一的资产库网格:点击才加载媒体(省内存) + 分类标签页(全部/视频/影片/图片/音频)。
    function AssetGrid(props) {
      ...
      const load = (rel) => setLoaded((prev) => ({ ...prev, [rel]: true }));
      ...
      const mediaStyle = { display: "block", width: "100%", height: 120, objectFit: "cover", background: "var(--dsw-alias-bg-base)" };
      const audioStyle = { display: "block", width: "100%", padding: "0 8px", boxSizing: "border-box" };
      const nameStyle = { margin: 0, padding: "6px 8px", fontSize: 11, color: "var(--dsw-alias-label-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
      const badgeStyle = { position: "absolute", top: 6, left: 6, fontSize: 10, fontWeight: 600, padding: "1px 6px", borderRadius: 999, color: "#fff", background: "rgba(0,0,0,0.55)" };
      const phWrap = { width: "100%", height: 120, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, background: "var(--dsw-alias-bg-layer-2, rgba(0,0,0,0.05))", cursor: "pointer" };
      const phIcon = { fontSize: 16, color: "var(--dsw-alias-label-secondary)" };
      const phText = { margin: 0, fontSize: 11, color: "var(--dsw-alias-label-tertiary)" };
      ...
      const cells = shown.map((asset) => {
        ...
        if (asset.kind === "video") {
          ...
          overlay = react.createElement("span", { style: hintStyle }, "▶ 单击预览 / 双击放大");
        } else if (asset.kind === "audio") {
          media = react.createElement("audio", { controls: true, preload: "metadata", src, style: audioStyle });
        } else {
        ...
        return react.createElement(
          "div",
          {
            ...
            onDoubleClick: () => setView(asset),
            title: "单击预览: " + (asset.name ?? rel),
            role: "button",
            tabIndex: 0,
          },
          ...
          react.createElement(
            ...
            react.createElement("p", { style: { margin: 0, fontSize: 11, color: "var(--dsw-alias-label-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, title: asset.name ?? "" }, asset.name ?? ""),
            asset.project ? react.createElement("p", { style: { margin: "2px 0 0", fontSize: 10, color: "var(--dsw-alias-label-tertiary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, title: asset.project }, pathBase(asset.project) || asset.project) : null,
          ),
        );
      });
    ...
    function ProjectAssets(props) {
      ...
      const gridStyle = { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 10 };
      const cellStyle = { borderRadius: 10, border: "1px solid var(--dsw-alias-border-l2)", overflow: "hidden", background: "var(--dsw-alias-bg-base)" };
      const mediaStyle = { display: "block", width: "100%", height: 110, objectFit: "cover", background: "var(--dsw-alias-bg-base)" };
      const audioStyle = { display: "block", width: "100%", padding: "0 8px", boxSizing: "border-box" };
      const nameStyle = { margin: 0, padding: "5px 8px", fontSize: 11, color: "var(--dsw-alias-label-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
      const badgeStyle = { position: "absolute", top: 6, left: 6, fontSize: 9, fontWeight: 600, padding: "1px 5px", borderRadius: 999, color: "#fff", background: "rgba(0,0,0,0.55)" };
      const hintStyle = { margin: 0, fontSize: 12, color: "var(--dsw-alias-label-tertiary)", fontStyle: "italic" };

      const cells = assets.map((asset) => {
        const src = asset.url ?? "";
        const key = asset.rel ?? asset.name ?? asset.url;
        let media;
        if (asset.kind === "video") {
          media = react.createElement("video", { controls: true, preload: "metadata", src, style: mediaStyle });
        } else if (asset.kind === "audio") {
          media = react.createElement("audio", { controls: true, preload: "metadata", src, style: audioStyle });
        } else {
          media = react.createElement("img", { src, alt: asset.name ?? "", style: mediaStyle, loading: "lazy" });
        }
        return react.createElement(
          "div",
          { key, style: { ...cellStyle, position: "relative" } },
          react.createElement("span", { style: badgeStyle }, asset.kind ?? "?"),
          media,
      ...
      if (loading && assets.length === 0) {
        ...
        body = react.createElement(AssetGrid, { assets });
      }
    ...
    function NewapiImageRow(props) {
      ...
      if (typeof args.n === "number" && args.n > 0) paramChips.push(["张数", `${args.n}`]);
    ...
    // -----------------------------------------------------------------------
    // 「视频创作模式」开关(持久化 + 跨组件共享):开=显示上传素材/媒体引用;
    // 关=隐藏插件在输入区的入口,用户可随时快速开关。
    // -----------------------------------------------------------------------
    const CREATE_MODE_KEY = "dsh-newapi-video.createMode";
    function readCreateMode() {
      try {
        return localStorage.getItem(CREATE_MODE_KEY) !== "off";
      } catch {
        return true;
      }
    }
    let createModeOn = readCreateMode();
    const createModeSubs = new Set();
    ...
    function setCreateMode(on) {
      createModeOn = !!on;
      try {
        localStorage.setItem(CREATE_MODE_KEY, createModeOn ? "on" : "off");
      } catch {
        // ignore storage errors
      }
      try {
        const r = fetch("/newapi/mode", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ enabled: createModeOn }),
        });
        r.catch(() => {});
      } catch {
        // ignore network errors; server guard will still enforce on next call
      }
      for (const fn of createModeSubs) fn(createModeOn);
    }
    function subscribeCreateMode(fn) {
      createModeSubs.add(fn);
      return () => createModeSubs.delete(fn);
    }
    function useCreateMode() {
      const [on, setOn] = react.useState(getCreateMode());
      react.useEffect(() => subscribeCreateMode(setOn), []);
      return on;
    }
    // 启动时与服务端权威状态同步一次(默认「开」)。
      ...
      try {
        fetch("/newapi/mode")
          ...
          .then((d) => {
            ...
            if (next !== createModeOn) {
              createModeOn = next;
              try {
                localStorage.setItem(CREATE_MODE_KEY, next ? "on" : "off");
              } catch {
                // ignore
              }
              for (const fn of createModeSubs) fn(next);
            }
          })
          .catch(() => {});
      } catch {
    ...
    // -----------------------------------------------------------------------
    // 上传素材按钮:点开系统文件选择器,把选中的图片/视频传到素材库目录
    // -----------------------------------------------------------------------
    function NewapiUploadButton(props) {
      // 「上传素材」已移除:素材引用交给 DSH 自带的附件上传(@附件)。本组件只保留「视频创作」开关。
      const createOn = useCreateMode();

      // 「视频创作」开关外观:开启=蓝紫渐变胶囊 + 滑块右移,关闭=灰 + 滑块左移,切换平滑过渡
      const modePillStyle = {
        display: "inline-flex",
        alignItems: "center",
        gap: 7,
        ...
        border: createOn ? "1px solid transparent" : "1px solid var(--dsw-alias-border-l2)",
        background: createOn
          ? "linear-gradient(135deg, #7c5cfc 0%, #4f8cff 100%)"
          : "var(--dsw-alias-bg-layer-3)",
        color: createOn ? "#fff" : "var(--dsw-alias-label-secondary)",
        boxShadow: createOn ? "0 1px 10px rgba(99,120,255,0.35)" : "none",
        transition: "background 240ms ease, color 240ms ease, border-color 240ms ease, box-shadow 240ms ease",
        whiteSpace: "nowrap",
        flex: "none",
      };
      const modeTrackStyle = {
        position: "relative",
        width: 30,
        height: 17,
        borderRadius: 999,
        flex: "none",
        background: createOn ? "rgba(255,255,255,0.34)" : "var(--dsw-alias-border-l2)",
      ...
      const modeKnobStyle = {
        ...
        height: 13,
        borderRadius: 999,
        background: createOn ? "#fff" : "var(--dsw-alias-label-secondary)",
        boxShadow: "0 1px 2px rgba(0,0,0,0.28)",
        transform: createOn ? "translateX(13px)" : "translateX(0)",
        transition: "transform 260ms cubic-bezier(0.4, 0, 0.2, 1), background 240ms ease",
      };
      return react.createElement(
        "div",
        ...
        react.createElement(
          ...
          {
            type: "button",
            onClick: () => setCreateMode(!createOn),
            title: createOn ? "关闭视频创作模式" : "开启视频创作模式",
            style: modePillStyle,
          },
          react.createElement("span", { style: { lineHeight: 1 } }, "视频创作"),
          react.createElement(
            "span",
            { style: modeTrackStyle },
            react.createElement("span", { style: modeKnobStyle }),
          ),
        ),
      );
    }
    ...
    function apply(ctx) {
      ...
      //    复用 NewapiAssetsSection 的内容,但外面包一层可滚动容器适应该视图。
      const AssetLibraryPanel = NewapiAssetsSection();
      function AssetsConversationView() {
        return react.createElement(
          "div",
          { style: { height: "100%", overflow: "auto", background: "var(--dsw-alias-bg-base)" } },
          react.createElement(AssetLibraryPanel),
        );
      }
      // 资产库标签页随「视频创作模式」开关动态挂载/卸载。
      // conversation.view 一旦注册,标签就会常驻(与组件是否返回 null 无关),
      // 所以要真正「去掉」它,必须在关闭时卸载插槽、开启时重新挂载。
      let assetsViewDispose = null;
      const registerAssetsView = () => {
        if (assetsViewDispose !== null) return;
        assetsViewDispose = ctx.slots.inject("conversation.view", () => {
          const dispose = ctx.slots.register(
      ...
      const unregisterAssetsView = () => {
        if (assetsViewDispose === null) return;
        assetsViewDispose();
        assetsViewDispose = null;
      };
      subscribeCreateMode((on) => (on ? registerAssetsView() : unregisterAssetsView()));
      if (getCreateMode()) registerAssetsView();
      ...
      ctx.slots.inject("conversation.input.dock", () => {
        const dispose = ctx.slots.register({ name: "conversation.input.dock", id: NS + "-preview", order: 100 }, NewapiComposerPreview);
        return dispose;
      });

      // 5) 输入框左侧「视频创作」开关(素材引用由 DSH 自带附件上传 @ 完成,已移除自定义上传按钮)
      ctx.slots.inject("conversation.input.left", () => {
        const dispose = ctx.slots.register({ name: "conversation.input.left", id: NS + "-upload", order: 200 }, NewapiUploadButton);
        return dispose;
      });
    }
